...
def newBackendWithCollection(collection):
    ...
    if backend_type == ab.Type.MxNet:
        from serving.backend import mxnet_python as mxnet
        return mxnet.MxNetBackend(collection, {
            'preheat': utils.getKey('be.mxpy.preheat', dicts=settings),
            'mxnet_identifier_suffix': utils.getKey('mxnet_identifier_suffix', dicts=settings),
            'face_image_size': utils.getKey('face_image_size', dicts=settings),
            'mxnet_json_path': utils.getKey("mxnet_json_path", dicts=settings),
