...
from serving.core import runtime, error_code
...
def inferenceLocal(data):
    ...
    if backend_instance is None:
        raise error_code.InferenceDataError(msg="failed to find backend")
    print(data)
    data_id = data.get('uuid')
    data_src = data.get('path')
    if data_id is None or data_src is None:
        raise error_code.InferenceDataError(msg="lack of data")
...
def inferenceRemote(data):
    ...
    if ret['code'] != 0:
        raise error_code.InferenceDataError(msg="failed to dump remote data")
