...
from serving.plugin.reader import reader
from serving.plugin.trigger import tragger
...
class Reader(re_pb2_grpc.ReaderServicer):
    def GetReader(self, request, context):
        ...
        try:
            p1 = Process(target=reader, args=(data,))
            p2 = Process(target=tragger, args=(data,))
            p1.start()
            p2.start()
